<?php
include('includes/config.php');
if(!empty($_POST["classid"])) 
{
 $cid=intval($_POST['classid']);
 if(!is_numeric($cid)){
 
 	echo htmlentities("invalid Class");exit;
 }
 else{
 $stmt = $dbh->prepare("SELECT StudentName,StudentId FROM tblstudents WHERE ClassId= :id order by StudentName");
 $stmt->execute(array(':id' => $cid));
 ?><option value="">Select Category </option><?php
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {
  ?>
  <option value="<?php echo htmlentities($row['StudentId']); ?>"><?php echo htmlentities($row['StudentName']); ?></option>
  <?php
 }
}

}
// Code for Subjects
if(!empty($_POST["classid1"])) 
{
 $cid1=intval($_POST['classid1']);
 if(!is_numeric($cid1)){
 
  echo htmlentities("invalid Class");exit;
 }
 else{
  session_start(); // Add this if not already at the top
  $status = 0;
  
  if (isset($_SESSION['tlogin'])) {
      $teacherId = $_SESSION['tlogin'];
  
      $stmt = $dbh->prepare("
          SELECT s.SubjectName, s.id 
          FROM tblsubjectcombination sc
          JOIN tblsubjects s ON s.id = sc.SubjectId
          JOIN teacher_subject_map tsm ON tsm.subjectId = s.id
          JOIN teachers t ON t.userId = tsm.teacherId
          WHERE sc.ClassId = :cid AND t.userId = :tid AND sc.status != :stts
          ORDER BY s.SubjectName
      ");
      $stmt->execute(array(':cid' => $cid1, ':tid' => $teacherId, ':stts' => $status));
  } else {
      // Admin logic (show all subjects)
      $stmt = $dbh->prepare("
          SELECT s.SubjectName, s.id 
          FROM tblsubjectcombination sc
          JOIN tblsubjects s ON s.id = sc.SubjectId
          WHERE sc.ClassId = :cid AND sc.status != :stts
          ORDER BY s.SubjectName
      ");
      $stmt->execute(array(':cid' => $cid1, ':stts' => $status));
  }
  
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ?>
      <p><?php echo htmlentities($row['SubjectName']); ?>
      <input type="text" name="marks[]" value="" class="form-control" required="" placeholder="Enter marks out of 100" autocomplete="off"></p>
  <?php
  }
  
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {?>
  <p> <?php echo htmlentities($row['SubjectName']); ?><input type="text"  name="marks[]" value="" class="form-control" required="" placeholder="Enter marks out of 100" autocomplete="off"></p>
  
<?php  }
}
}


?>

<?php

if(!empty($_POST["studclass"])) 
{
 $id= $_POST['studclass'];
 $dta=explode("$",$id);
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT StudentId,ClassId FROM tblresult WHERE StudentId=:id1 and ClassId=:id ");
//$query= $dbh -> prepare($sql);
$query-> bindParam(':id1', $id1, PDO::PARAM_STR);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query-> execute();
$results = $query -> fetchAll(PDO::FETCH_OBJ);
$cnt=1;
if($query -> rowCount() > 0)
{ ?>
<p>
<?php
echo "<span style='color:red'> Result Already Declare .</span>";
 echo "<script>$('#submit').prop('disabled',true);</script>";
 ?></p>
<?php }


  }?>


